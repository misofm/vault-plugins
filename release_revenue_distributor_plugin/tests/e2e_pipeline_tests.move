...
fun assert_no_admin_caps(scenario: &Scenario) {
    assert!(!scenario.has_most_recent_for_sender<CompositionAdminCap<COMPOSITION_SHARE>>());
    assert!(!scenario.has_most_recent_for_sender<RecordingAdminCap<RECORDING_SHARE>>());
    assert!(!scenario.has_most_recent_for_sender<ReleaseAdminCap>());
    assert!(!scenario.has_most_recent_for_sender<VaultAdminCap<RecordingAdminCap<RECORDING_SHARE>>>());
...
fun release_plugin_to_recording_action_to_routed_composition_pool() {
    ...
    let (mut recording, recording_admin_cap) =
        recording::new_for_testing<RECORDING_SHARE, COMPOSITION_SHARE>(composition_id, scenario.ctx());
    ...
    let mut composition_pool = composition_pool_action::new_pool<COMPOSITION_SHARE, CURRENCY>(
        &mut composition, &composition_admin_cap,
    );
    ...
    let mut recording_pool = recording_pool_action::new_pool<RECORDING_SHARE, COMPOSITION_SHARE, CURRENCY>(
        &mut recording, &recording_admin_cap,
    );
    ...
    balance::create_for_testing<RECORDING_SHARE>(100).send_funds(composition_id.to_address());
    ...
    scenario.next_tx(ADMIN);
    let mut recording = scenario.take_shared<Recording<RECORDING_SHARE, COMPOSITION_SHARE>>();
    let mut recording_vault = scenario.take_shared<Vault<RecordingAdminCap<RECORDING_SHARE>>>();
    let recording_vault_admin_cap = scenario.take_from_sender<VaultAdminCap<RecordingAdminCap<RECORDING_SHARE>>>();
    ...
    let composition_admin_cap = scenario.take_from_sender<CompositionAdminCap<COMPOSITION_SHARE>>();
