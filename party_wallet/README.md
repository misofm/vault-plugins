...
- `is_installed` and `inbox_address`: Read-only views.
...
- Funds and objects: the Vault administrator selects receiving tickets or an
  exact accumulator amount. Object recipients remain explicit; returned
  balances can be routed only by the PTB that supplied the matching Vault
  authority.
...
- A client can call
  `sui::balance::settled_funds_value<Currency>(AccumulatorRoot, Party ID)` and
  pass its `u64` result directly to `redeem_balance` in the same PTB. The read
  is commit-settled and excludes funds credited earlier in the current commit.
  A zero result aborts in `hikida`, and the framework reports at most
  `u64::MAX` per call.
- Competing PTBs can read the same snapshot; after one redeems it, another can
  abort during withdrawal rather than overdraw the accumulator.
...
- The Move unit-test VM does not populate funded `AccumulatorRoot` snapshots.
  Exact, partial, overdraw, event, and authorization behavior is unit-tested. A
  test-only scenario feeds the framework reader's zero result into
  `redeem_balance`; a positive settled read and actual PTB command-result chain
  should also be checked on-chain before production reliance.
